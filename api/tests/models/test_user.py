...
class TestProfileObject(TestCase):
    ...
    def setUp(self):
        ...
        self.profile = Profile.objects.get(name=USER_NAME)
        self.image = create_image(profile=self.profile)
        self.profile.featured_image = self.image
        self.profile.name = PROFILE_NAME
        self.profile.location = PROFILE_LOCATION
        self.profile.silph_card = PROFILE_SILPH_CARD_URL
        self.profile.save()
...
class TestDefaultProfileObject(TestCase):
    """ Test the profile object created when creating user """

    def setUp(self):
        """ Set up the tests """
        super(TestDefaultProfileObject, self).setUp()
        self.user = User.objects.create(username=USER_NAME)
        self.profile = Profile.objects.get(name=USER_NAME)
        self.nowish = datetime.now()
    ...
    def test_name(self):
        ...
        Test that name passed on creation is saved to the object
        """
        self.assertEqual(self.profile.name, USER_NAME)
    ...
    def test_silph_card(self):
        ...
        Test that silph card passed on creation is saved to the object
        """
        self.assertEqual(self.profile.silph_card, '')

    def test_featured_image(self):
        """
        Test that featured_image saved to profile is saved to the object
        """
        self.assertIsNone(self.profile.featured_image)
